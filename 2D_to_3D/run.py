...
for i, image_path in enumerate(args.image):
    ...
    if args.no_remove_bg:
        ...
        image.save(os.path.join(output_dir, image_name, f"input.png"))
    images.append(image)
timer.end("Processing images")

for i, image in enumerate(images):
    image_name = image_names[i]
    logging.info(f"Running image {i + 1}/{len(images)} ({image_name}) ...")
    ...
    if args.render:
        ...
        for ri, render_image in enumerate(render_images[0]):
            render_image.save(os.path.join(output_dir, image_name, f"render_{ri:03d}.png"))
        save_video(
            render_images[0], os.path.join(output_dir, image_name, f"render.mp4"), fps=30
        )
        timer.end("Rendering")
    ...
    if args.bake_texture:
        out_mesh_path = os.path.join(output_dir, image_name, f"mesh.{args.model_save_format}")
        out_texture_path = os.path.join(output_dir, image_name, "texture.png")
        ...
        for fmt in ("obj", "glb"):
            meshes[0].export(os.path.join(output_dir, image_name, f"mesh.{fmt}"))
